...
sem_t buffer;			// mutes para acesso ao buffer
...
int main(int argc, char *argv[])
{
	...
	// inicia semaforos
	sem_init(&buffer, 0, 1); /*Semaphoro local -- nao pode ser acessado */
	sem_init(&vagas, 0, VAGAS); /**/
	sem_init(&itens, 0, 0);/*itens comecando em 0 */

	// cria produtores
	for(i=0; i< NUM_PROD; i++)
	{
		if(pthread_create(&produtor[i], NULL, prodBody, (void *) i)) /*Parametros join*/
		{
